...
public:
  TimeWheel(unsigned long long n) : mCurrentId(1), mSize(n), mCurrentSlot(0) {
    this->mWheel.resize(n);
  }
...
public:
  ...
  inline unsigned long long add(unsigned long long ticks, TaskFn fn) {
    ...
    this->mWheel[(this->mCurrentSlot + ticks) % this->mSize].emplace_back(
        tid, ticks / this->mSize, ticks, 1, fn);
    this->mRunningTasks.insert(tid);
    return tid;
  }
  inline void cancel(unsigned long long tid) { this->mCancelSet.insert(tid); }
  inline void cancelAll() {
    for (auto &i : this->mWheel)
      i.clear();
  }
  inline bool isRunning(unsigned long long tid) {
    return this->mRunningTasks.find(tid) != this->mRunningTasks.end();
  }
...
private:
  inline void add(Task task) {
    this->mWheel[(this->mCurrentSlot + task.mDuration) % this->mSize]
        .emplace_back(task.mTaskId, task.mDuration / this->mSize,
                      task.mDuration, task.mCallTimes + 1, task.mFn);
  }
